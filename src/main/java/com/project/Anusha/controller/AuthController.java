...
public class AuthController {
    ...
    public ResponseEntity<?> preparePasswordReset(@RequestBody Map<String, String> request) {
        ...
        try {
            userService.preparePasswordReset(email);
    ...
    public ResponseEntity<?> syncFirebasePassword(@RequestBody Map<String, String> request) {
        String idToken = request.get("idToken");
        String newPassword = request.get("newPassword");

        if (idToken == null || idToken.isBlank() || newPassword == null || newPassword.isBlank()) {
            return ResponseEntity.badRequest().body(
                    Map.of("message", "idToken and newPassword are required"));
        }
        ...
        try {
            ...
            String email = firebaseToken.getEmail();
            if (email == null || email.isBlank()) {
                return ResponseEntity.badRequest().body(Map.of("message", "Firebase token did not include an email"));
            }

            User user = userService.syncPasswordFromFirebase(email, newPassword);
            return ResponseEntity.ok(buildSuccessfulLoginResponse(user));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Map.of("message", e.getMessage()));
        }
    ...
    public ResponseEntity<?> changePassword(@RequestBody Map<String, String> request) {
        ...
        if (email == null || currentPassword == null || newPassword == null) {
            return ResponseEntity.badRequest().body(
                    Map.of("message", "email, currentPassword and newPassword are required"));
        }
        try {
            userService.changePasswordForAdmin(email, currentPassword, newPassword);
    ...
    public ResponseEntity<?> refreshToken(@RequestBody RefreshTokenRequest request) {
        try {
            RefreshTokenService.TokenPair tokenPair =
                    refreshTokenService.rotate(request.getRefreshToken(), RefreshToken.ClientType.ADMIN_PANEL);
    ...
    public ResponseEntity<?> logout(@RequestBody RefreshTokenRequest request) {
        refreshTokenService.revoke(request.getRefreshToken(), RefreshToken.ClientType.ADMIN_PANEL);
    ...
    private LoginResponse buildSuccessfulLoginResponse(User user) {
        RefreshTokenService.TokenPair tokenPair = refreshTokenService.issueForAdmin(user);
